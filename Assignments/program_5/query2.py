...
------------
    ...
    the pygame screen. The program starts by accepting system arguements from the command line.
    It will accept one which is a radius to be searched, or 6 arguments which can be used to
    filter search and display results. If one arguement is passed in, upon mouse click the pygame
    screen will display volcanos in red, earthquakes in blue, and meteorites in green in the
    radius determined by the arguement, they will show for 3 to 5 seconds and then vanish where
    the user may click on another area to repeat the process. If more arguements are passed in
    they can be used to filter search results when clicking on the pygame screen, they will also
    appear if there are any for 3 to 5 seconds and then vanish allowing the user to click and search
    again. A screenshot is captured after each search, however it is overwritten each search.
...
if __name__=='__main__':
    ...
    if len(sys.argv) != 2:
        ...
        #Earthquakes: mag, rms, year, depth, time
